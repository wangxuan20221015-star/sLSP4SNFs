...
class SLSP:
    ...
    def analyze_snf(self,freq,deltaF=0.1, order=1):
        """
        ...
        order : int, optional
            SNF order: 
            - order=1 for fny-2fny region (standard SNF, phase normal)
            - order=2 for 2fny-3fny region (double amplitude, phase reversed)
        ...
        For order=2 (2fny-3fny):
            - Modulation amplitude is doubled
            - Phase is reversed relative to theoretical prediction
    
        All frequency residuals used in the SNF criterion are expressed
        """
        ...
        """
        if order == 1:
            # Standard SNF (fny-2fny): normal phase
            residual = np.abs(
                (self.centerFreqs - self.targetFreq) * 1000.0 - 2.0 * self.snfs
            )    # nHz
        elif order == 2:
            # Higher-order SNF (2fny-3fny): doubled amplitude, reversed phase
            residual = np.abs(
                (self.centerFreqs - self.targetFreq) * 1000.0 + 4.0 * self.snfs
            )    # nHz
        else:
            raise ValueError(f"Unsupported SNF order: {order}. Use order=1 or order=2.")
    ...
    def _print_summary(self):
        ...
        if state == "Confirmed":
            msg = (
                f"SUCCESS: The target at {self.targetFreq:.2f} uHz is identified as a \n"
                f"Super-Nyquist Frequency (SNF). \n"
            )
        else:
            msg = (
                f"REJECTED: The target at {self.targetFreq:.2f} uHz is NOT an SNF. \n"
            )
    ...
    def _check_snf_state(self):
        ...
        is_period_valid = abs(self.snfPeriod - 372.0) < 5.0 * self.sigmaPeriod

        snf_order = getattr(self, "snfOrder", 1)
        if snf_order == 2:
            is_amp_strong = (self.snfSnr >= 3.0) and (self.snfAmp > 0.02)
        else:
            is_amp_strong = (self.snfSnr >= 3.0) and (self.snfAmp > 0.01)

        theory_strict = self.criterion <= 0.3
        theory_loose = 0.3 <= self.criterion <= 0.5

        self.snfState = (
            "Confirmed"
            if theory_strict or (is_period_valid and is_amp_strong and theory_loose)
            else "Rejected"
    )
